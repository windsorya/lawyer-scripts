 ...
 * v1.8 變更：(1) 新增從 Notion 查詢「▸當事人類型」欄位（自然人/法人/機關）
 *           (2) 法人/機關統一用新模板：律師前往開庭、不提攜帶文件、不提到場會合
 *           (3) 判斷優先順序：▸當事人類型 > 舊版機關正則 fallback > 自然人
 *           (4) 舊版 orgPattern 正則保留為 fallback（未標記▸當事人類型時使用）
 */
...
// ======================== v1.8 重構：Notion 案件資料查詢 ========================

/**
 * 從 Notion 案件追蹤資料庫批次查詢所有案件資料
 * 回傳格式：{ "金順案": { gender: "...", contact: "張小姐", partyType: "法人" }, ... }
 *
 * partyType: "自然人"/"法人"/"機關"/""（空字串＝未標記）
 */
function fetchCaseDataFromNotion_() {
  ...
  try {
    ...
    data.results.forEach(function(page) {
      ...
      // v1.8：取得當事人類型（select 欄位）
      if (props['▸當事人類型'] && props['▸當事人類型'].select && props['▸當事人類型'].select.name) {
        partyType = props['▸當事人類型'].select.name;
      }

      if (caseName) {
        map[caseName] = { gender: gender, contact: contact, partyType: partyType };
      }
    });
...
/**
 * v1.8：根據案件簡稱判斷稱呼和當事人類型
 * 回傳 { greeting, isNonNatural }
 * greeting: 稱呼文字（如「張小姐」「蘇先生」「您」）
 * isNonNatural: 是否為非自然人案件（法人/機關），影響訊息模板
 *
 * 判斷優先順序：
 * 1. Notion「▸當事人類型」欄位（最優先）
 * 2. 舊版機關正則 fallback（未標記時）
 * 3. 預設為自然人
 */
function getGreetingInfo_(caseName, caseDataMap) {
  ...
  // ★ v1.8：優先用 ▸當事人類型 欄位判斷
  if (caseData && caseData.partyType) {
    if (caseData.partyType === '法人' || caseData.partyType === '機關') {
      // 有承辦窗口 → 用承辦窗口稱呼；無 → 用「您」
      var orgGreeting = (caseData.contact) ? caseData.contact : '您';
      return { greeting: orgGreeting, isNonNatural: true };
    }
    // partyType === '自然人' → 走下方自然人邏輯
  }

  // ★ v1.8 fallback：未標記▸當事人類型時，用舊版機關正則
  if (!caseData || !caseData.partyType) {
    // 有承辦窗口 → 視為機關/法人
    if (caseData && caseData.contact) {
      return { greeting: caseData.contact, isNonNatural: true };
    }
    ...
    if (caseName.match(orgPattern)) {
      return { greeting: '您', isNonNatural: true };
    }
  }
  ...
  if (lastName) {
    ...
    return { greeting: lastName + honorific, isNonNatural: false };
  }

  return { greeting: '您', isNonNatural: false };
}
...
function buildNotifyClientMsg_(event, dateStr, dayOfWeek, timeStr, dayLabel, caseDataMap) {
  ...
  var isNonNatural = info.isNonNatural;
  ...
  // ★ v1.8：非自然人（法人/機關）統一模板
  if (isNonNatural) {
    var action = '開庭';
    if (hearingType === '宣判') action = '宣判';
    else if (hearingType === '調解') action = '進行調解';

    // v1.8.1：修正「您您好」bug——greeting 為「您」時直接用「您好」
    var greetingLine = (greeting === '您') ? '您好，提醒您' : greeting + '您好，提醒您';

    var msg = greetingLine + '\n\n'
      + '本案 ' + dateTimeStr + ' ' + action + '。\n\n'
      + '屆時律師會前往' + action + '，如有任何問題，請提前告知，並與律師保持聯繫，謝謝。';

    // 宣判時加結果通知
    if (hearingType === '宣判') {
      msg = greetingLine + '\n\n'
        + '本案 ' + dateTimeStr + ' 宣判。\n\n'
        + '屆時律師會前往開庭，律師會在宣判後第一時間告知結果。\n'
        + '如有任何問題，請提前告知，並與律師保持聯繫，謝謝。';
    }
  ...
  // ★ 自然人模板（維持原邏輯）
  // 通用：greeting 為「您」時直接用「您好」，避免「您您好」
  var greetingHello = (greeting === '您') ? '您好，' : greeting + '您好，';

...
- 2026-04-12 notionHighlightSync v1.0 部署：每 5 分鐘掃 Notion 💼工作待辦 DB（6e2e22f21ca4824ab71f0757cca88f8c），Checkbox 剛勾 true → 擷取關鍵詞 → GCal Highlight 行事曆找標題相符事件 → setColor(SAGE) 改綠；觸發器由 sendMorningBriefing 自動安裝（⏳待同步）
- 2026-04-12 notionHighlightSync v2.0 雙向同步：方向A(Notion Checkbox=true→GCal改綠)＋方向B(GCal綠色事件→Notion打勾)；各自防循環檢查（已達目標狀態→skip）；ensureHighlightSyncTriggers_() 安裝兩個5分鐘觸發器；程式碼.js 觸發器呼叫同步更新（⏳待同步）

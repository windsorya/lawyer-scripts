"""
bookmark_engine.py — 卷證書籤核心引擎 v2
...
_add(r'\d{2,3}年度[^\s]{1,10}字第\d+號', standalone=True)  # 案號需獨行（避免起訴書內多案號誤判）

# 個人/前案紀錄（L1，比「刑案資料」更具體先放）
# 含 OCR 異體字：查/査、註/注、紀/记
_add(r'前案紀錄表')
_add(r'刑案資料[查査][注註]紀錄表')  # OCR 異體字容錯
_add(r'在監在押紀[錄録]表')
_add(r'戶籍謄本')
...
# 筆錄（L1，具體→一般；generic「筆錄」加 standalone 防誤判）
_add(r'審判筆錄')
_add(r'準備程序筆錄')
_add(r'訊問筆錄')
_add(r'詢問筆錄')
...
_add(r'勘驗筆錄')
_add(r'扣押筆錄')
_add(r'筆錄', standalone=True)  # 單獨「筆錄」易誤判，需獨行

# 裁判（L1，standalone=True：防止正文中的「判決」「裁定」誤判）
...
_add(r'刑事案件報告書?')
_add(r'職務報告')
_add(r'報告書', standalone=True)  # 通用「報告書」需獨行，避免「報告書編號：xxx」誤判

# 結文（L1；「鑑定人結文」比「結文」具體，先放）
_add(r'鑑定人結文')
_add(r'結文', standalone=True)  # 單獨「結文」需獨行，避免誤判
...
_add(r'附錄[^。\n]{0,30}',          is_attachment=True)  # 附錄資料:xxx
_add(r'附件[一二三四五六七八九十百千\d][^\s。\n]{0,10}', is_attachment=True)  # 附件十九、附件15_
_add(r'附件\d+',                     is_attachment=True)  # 附件5
_add(r'附件',                        is_attachment=True)  # 單純附件
...
def _scan_page(page: fitz.Page) -> list[tuple[str, bool]]:
    """
    掃描頁面，回傳「按頁面順序」匹配到的書籤列表。
    每頁上限：1 個 L1（文件）+ 1 個 L2（附件/附錄），合計最多 2 個。
    這樣能捕捉「附件5 + 數位證物勘察採證同意書 同頁」的情況，
    同時避免目錄頁、多案號頁造成的多重誤判。
    """
    lines = _get_all_lines(page)
    l1_match: Optional[tuple[str, bool]] = None  # 第一個 L1 (is_attachment=False)
    l2_match: Optional[tuple[str, bool]] = None  # 第一個 L2 (is_attachment=True)
...
def generate_bookmarks(
    ...
    """
    讀取 input_path，自動辨識卷證結構，寫入書籤，存到 output_path。
